		...
		try {
			response = await pool.query(
				...
				LEFT JOIN event_registrations er
					ON er.event_id = e.id
					AND er.user_id = $1
				WHERE e.is_deleted = false AND e.status IN ('published', 'upcoming', 'ongoing')
		...
		const formattedEvent = {
			...
			registration: {
				config: {
					...
					participationType: event.registration_rules?.[0]?.participation || null,
				},
				rules: event.registration_rules || [],
			},
			team: {
				enabled: event.registration_rules?.[0]?.participation === "team" && event.registration_rules?.[0]?.allow,
			...
			stats: { totalRegistrations: Number(event.total_registrations) || 0, totalTeams: Number(event.total_teams) || 0 },
			meta: { status: event.status, visibility: event.visibility, createdAt: event.created_at },
		};
