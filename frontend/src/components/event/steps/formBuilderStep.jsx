...
import { FileText, Type, Hash, Mail, List, Circle, Plus, Trash2, Eye, Phone, Link, Calendar, CheckSquare, Upload, User, FileCheck, LayoutTemplate } from "lucide-react";
...
const TEMPLATES = {
	...
	workshop: [
		{ label: "Current Skill Level", type: "select", required: true, options: ["Beginner", "Intermediate", "Advanced"] },
		{ label: "Why do you want to attend?", type: "textarea", required: true },
		{ label: "Dietary Preferences", type: "select", required: false, options: ["Veg", "Non-veg", "Vegan"] },
	],
	cultural: [
		{ label: "Performance Category", type: "select", required: true, options: ["Solo", "Group", "Duo"] },
		{ label: "Performance Duration (mins)", type: "number", required: true },
...
const FormBuilderStep = ({ eventData, setEventData }) => {
	...
	const updateField = (id, prop, value) => {
		setFields((prev) =>
			prev.map((f) => {
				...
				// Handle type changes
				if (prop === "type") {
					const needsOptions = value === "select" || value === "checkbox";
					const hadOptions = f.type === "select" || f.type === "checkbox";
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-center gap-3 pb-4 border-b border-slate-100">
				...
				<div>
					<h3 className="text-lg font-semibold text-slate-900">Registration Form Builder</h3>
					<p className="text-sm text-slate-500">Define what additional information participants must provide while registering</p>
				</div>
			</div>
			...
			<div className="border-2 border-slate-200 rounded-xl p-5 bg-slate-50">
				<div className="flex items-center gap-2 mb-4">
					...
					<h4 className="font-medium text-slate-700">Mandatory System Fields</h4>
				</div>
				<p className="text-xs text-slate-500 mb-3">These fields are always included and cannot be removed</p>
				
				<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
					{SYSTEM_FIELDS.map((field) => (
						<div key={field.label} className="flex items-center gap-3 bg-white border border-slate-200 rounded-lg px-4 py-3">
							<input
								type="checkbox"
									...
									<div className="space-y-2">
										{(field.options || []).map((opt, optIndex) => (
											<div key={optIndex} className="flex items-center gap-2">
												<span className="text-xs text-slate-400 w-6">{optIndex + 1}.</span>
												<input
													type="text"
			...
			{fields.length === 0 && (
				<div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center">
					...
					<p className="text-slate-500 text-sm mb-2">No custom fields added yet</p>
					<p className="text-slate-400 text-xs">Click "Add Field" or select a template to get started</p>
				</div>
			)}
			...
			<div className="border-2 border-slate-200 rounded-xl p-5 bg-slate-50">
				<div className="flex items-center gap-2 mb-4">
					...
					<h4 className="font-medium text-slate-800">Registration Form Preview</h4>
				</div>
