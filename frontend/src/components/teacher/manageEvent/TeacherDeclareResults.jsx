...
const TeacherDeclareResults = ({ event }) => {
	...
	const [formData, setFormData] = useState(
		Array.from({ length: configuredPositions }, (_, index) => ({
			position: index + 1,
			participant: "",
			rankLabel: index === 0 ? "Winner" : `${index + 1}${index === 1 ? "nd" : "th"} Place`,
			score: "",
			remarks: "",
		})),
	);
		...
		try {
			await createEventResults(
				...
				formData.map((result) => ({
					...
					registrationId: event.team.enabled ? null : result.participant,
			...
			toast.error(error.response?.data?.message || "Failed to declare results");
		} finally {
			setSaving(false);
		}
	};
	...
	<div key={result.position} className="rounded-xl border border-slate-200 p-4">
		...
		<select
			...
			onChange={(e) => updateResult(index, "participant", e.target.value)}
...
{participants.map((participant) => (
	<option
					key={event.team.enabled ? participant.team_id : participant.registration_id}
		value={
			event.team.enabled
				? participant.team_id
	...
	>
		...
		<input
			...
			onChange={(e) => updateResult(index, "rankLabel", e.target.value)}
