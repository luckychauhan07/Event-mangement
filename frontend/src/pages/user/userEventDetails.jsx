...
const UserEventDetails = () => {
	...
	const registrationState = useMemo(() => {
		...
		if (isRegisteredByUser(eventDetails)) {
			return { canRegister: false, message: getRegistrationLabel(eventDetails) };
		}

		const now = new Date();
		const registrationStart = registration.start ? new Date(registration.start) : null;
		const registrationEnd = registration.end ? new Date(registration.end) : null;
		...
		if (eventDetails.meta.status === "ongoing" || eventDetails.meta.status === "completed") {
			return {
				canRegister: false,
	...
	const handleRegister = async () => {
		try {
			setRegistering(true);
			// Use the correct user-specific registration route
					...
					<div className="mt-6 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
						<div className="max-w-3xl">
							...
							<p className="mt-3 max-w-2xl text-sm leading-6 text-white/85">
								{basic.description || "No description provided."}
							</p>
						</div>
					...
					<div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
						...
						<div className="mt-5 grid gap-4 md:grid-cols-2">
							...
							<div className="rounded-2xl bg-slate-50 p-4">
								...
								<p className="mt-2 text-sm text-slate-700">
									{basic.entryFee ? `Rs. ${basic.entryFee}` : "Free"}
								</p>
							</div>
							<div className="rounded-2xl bg-slate-50 p-4">
								<p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
					...
					<div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
						...
						<div className="mt-5 grid gap-4 md:grid-cols-2">
							...
							<div className="rounded-2xl bg-slate-50 p-4">
								...
								<p className="mt-2 text-sm text-slate-700">
									{registration.participationType || "Individual"}
								</p>
							</div>
							<div className="rounded-2xl bg-slate-50 p-4">
								<p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
				...
				<div className="space-y-6">
					<div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
						...
						<div className="mt-6 flex flex-col gap-3">
							{isWithdrawalAllowed(eventDetails) ? (
								<button
									type="button"
								...
								<button
									...
									disabled={!registrationState.canRegister || registering}
							...
							{coordinators.length === 0 ? (
								...
								coordinators.map((coordinator) => (
									...
									>
										...
										<p className="mt-1 text-sm text-slate-600">
											{coordinator.phone || "Phone not available"}
										</p>
									</div>
								))
							)}
