...
const UserEvents = () => {
	...
	const addTeamMember = () => {
		...
		if (totalCountIfAdded > maxTeamSize) {
			setTeamFormError(
				`You can add up to ${maxTeamSize} total team members.`,
			);
			return;
		}
	...
	const renderStatusPill = (event) => {
		...
		return (
			<span
				className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${pillClass}`}
			>
				{icon}
				{statusLabel}
			</span>
	...
	const renderEventCard = (event) => {
		...
		if (isRegistered) {
			regState = {
				canRegister: false,
				message: getRegistrationLabel(event),
			};
			...
			regState = { canRegister: false, message: "" };
				...
				<div className="space-y-5 p-6">
					...
					<div className="rounded-2xl border border-blue-100 bg-blue-50/60 px-4 py-3 text-sm text-slate-600">
						<div className="flex items-start gap-2">
							...
							<p>
								...
								coordinator information, registration rules, and
								the complete event schedule before joining.
							</p>
						</div>
					</div>
					...
					<div className="flex flex-wrap gap-3">
						...
						!isRegistered ? null : isRegistered &&
						  isWithdrawalAllowed(event) ? ( // Show withdraw button only if registered and withdrawal allowed
							...
							<button
								...
								onClick={() =>
									navigate(
										`/user/events/${event.id}/register`,
									)
								}
								disabled={
									!regState.canRegister || isRegistering
								}
				...
				<div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
					<div className="max-w-2xl">
						...
						<p className="mt-3 text-sm leading-6 text-blue-50">
							Explore upcoming opportunities, review event
							details, and register directly from the user module.
						</p>
					</div>
							...
							<section className="space-y-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-slate-500">
											Upcoming and ongoing events open for
											students
										</p>
									</div>
								...
								{availableEvents.length === 0 ? (
									<div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-sm text-slate-500">
										No available events match the current
										filters.
									</div>
								) : (
							...
							<section className="space-y-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-slate-500">
											Previously completed events with
								...
								{completedEvents.length === 0 ? (
									<div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-sm text-slate-500">
										You do not have any completed events
										yet.
									</div>
								) : (
									<div className="grid gap-5 xl:grid-cols-2">
						...
						<div className="flex items-start justify-between gap-4">
							<div>
								...
								<p className="mt-2 text-sm text-slate-600">
									Add the team name, leader details, and
									member list before submitting the team
									registration.
								</p>
							</div>
						...
						<div className="mt-6 grid gap-4 md:grid-cols-2">
							...
							<div className="rounded-2xl bg-slate-50 p-4">
								...
								<p className="mt-2 text-sm font-medium text-slate-800">
									...
									{teamModalEvent.max_team_size || "N"}{" "}
									members
								</p>
							</div>
						</div>
